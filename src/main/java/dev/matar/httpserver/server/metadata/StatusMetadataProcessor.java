...
public class StatusMetadataProcessor implements HttpMetadataProcessor {
  private final ConnectionMetadataProcessor connectionMetadataProcessor;

  public StatusMetadataProcessor() {
    this.connectionMetadataProcessor = new ConnectionMetadataProcessor();
  }
  ...
  public void process(HttpRequest request, HttpResponse<?> response) {
    ...
    if (status >= 400 && status <= 599) {
      ...
      connectionMetadataProcessor.process(request, response);
    }
  }
}
